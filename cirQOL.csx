...
if (Data == null) {
    ...
    if (File.Exists(circloO_filepath)) {
        ...
        object[] args = new object[] {circloO_filepath, false, false};
...
void newPatch(string id, string name, Action patch) {
    patches.Add(name);
    patchids.Add(id);
    patchfunctions.Add(patch);
}
void newPatch(string id, Action patch) {
    patches.Add(id);
    patchids.Add(id);
    patchfunctions.Add(patch);
}
void newPreset(string name, List<string> patches) {
    presets.Add(name);
    presetpatches.Add(patches);
}
void newPreset(string name) {
    presets.Add(name);
    presetpatches.Add(new List<string> {});
}
...
public class cirQOLdialog : System.Windows.Forms.Form
{
    ...
    public cirQOLdialog(List<string> selectedPatches, List<string> patches, List<string> patchids, List<string> presets, List<List<string>> presetpatches, string circloO_filepath, UndertaleData Data) {
        this.Data = Data;
...
"Credits");
    ...
    public void itemCheck(object sender, ItemCheckEventArgs e) {
        ...
        this.AcceptButton.Enabled = enableAcceptButton;
        this.styleButtonGreen(this.AcceptButton, !enableAcceptButton);
    }
}

// DO NOT ENABLE THIS IF YOU ARE ON THE BETA!!! it WILL NOT WORK. this is here for compatibility
...
// PRESETS
List<string> QOLmods = new List<string> {};
List<string> editmods = new List<string> {};
List<string> recommended = new List<string> {};
List<string> experimental = new List<string> (recommended);
...
newPreset("Experimental Features", experimental);
newPreset("Recommended", recommended);
newPreset("QOL modifications only", QOLmods);
newPreset("Editor modifications only", editmods);
newPreset("Nothing", new List<string> {});

public Form form = new cirQOLdialog(selectedPatches, patches, patchids, presets, presetpatches, circloO_filepath, Data);
form.ShowDialog();
if (selectedPatches.Count == 0) return;

PATCHLIST = String.Join("\n", selectedPatches);
...
notification_set(""cirQOL "+VERSION+" loaded with "+selectedPatches.Count+(selectedPatches.Count > 1 ? " patches!" : " patch!")+@"\nPress F1 to show the help menu"", 10)
", Data);
...
for (int i = 0; i < patchids.Count; i++) {
    if (selectedPatches.Contains(patchids[i])) {
        patchfunctions[i]();
    }
}
...
// fuck you private methods, ill do what i want
object[] args = new object[] {circloO_filepath, false};
